...
# 最初のページ
# FastAPIのレスポンスはデフォルトでJSON（response_class=JSONResponse）
...
# デフォルト引数の設定（例：駅からの距離=stattion_distの引数が与えられなかった時は0.0とする）
async def predict_price(request:Request,
    ...
    predicted_price = round(predicted[0], 6) # StandardScalerの戻り値は配列（ndarray）なので[0]を指定
